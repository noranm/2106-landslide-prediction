...
for (sgg in KN_SGGs) {
  ...
  for (dt in DATEs){
    ...
    for (f in FILEs) {
      ...
      if (value == "강수") {
        tmp1 <- CSVfile %>% group_by(day) %>% summarise(var1 = max(val, na.rm=TRUE),
                                                        var2 = sum(val, na.rm=TRUE)) %>% data.frame()
        colnames(tmp1) <- c("day", "최대시우량", "하루총강수량")
        tmp1$SIDO <- "경상남도"; tmp1$SGG_NM <- sgg; tmp1$UMD <- umd; 
        tmp1$날짜 <- as.character(as.numeric(first_date)*100 + tmp1$day); 
        final1 <- rbind(final1, tmp1)
      } else if (value == "풍속") {
        tmp2 <- CSVfile %>% group_by(day) %>% summarise(var = max(val, na.rm=TRUE)) %>% data.frame()
        colnames(tmp2) <- c("day", "최대풍속")
        tmp2$SIDO <- "경상남도"; tmp2$SGG_NM <- sgg; tmp2$UMD <- umd; 
        tmp2$날짜 <- as.character(as.numeric(first_date)*100 + tmp2$day);
        final2 <- rbind(final2, tmp2)
      } else {
        # 없음(0), 비(1), 비/눈(2), 눈(3)
        tmp3 <- CSVfile %>% group_by(day) %>% summarise("var1" = sum(val != 0, na.rm=TRUE)/24) %>% data.frame()
        
        colnames(tmp3) <- c("day", "강수비율")
        tmp3$SIDO <- "경상남도"; tmp3$SGG_NM <- sgg; tmp3$UMD <- umd; 
        tmp3$날짜 <- as.character(as.numeric(first_date)*100 + tmp3$day);
...
colSums(is.na(final))
final <- final[!is.na(final$날짜),]

file_format <- file("./WeatherInfo.csv", encoding='euc-kr')
